...
import { onRequest } from "../../functions/api/mint";
...
const Connected = (props: { userAddress: Address; chainId: number }) => {
  ...
  const history = useGetTransactionHistory({
    chainId,
  ...
  useEffect(() => {
    const historyRefetch = setInterval(() => {
      history?.refetch();
      console.log("refetch");
    }, 6_000);

    return () => clearInterval(historyRefetch);
  }, []);
  ...
  const [packQueue, setPackQueue] = useState<boolean[]>([]);

  function addToQueue() {
    setPackQueue((prevItems) => [...prevItems, true]);
  }
  ...
  function nextInQueue() {
    if (packQueue.length > 0) {
      ...
      setPackQueue((prevItems) => prevItems.slice(1));
    }
  }
  ...
  useEffect(() => {
    ...
    if (commit.status !== "pending" && packQueue.length > 0) nextInQueue();
  }, [packQueue, commit.status]);
  ...
  //       {Array(Math.min(chestsRemaining, 3))
  //         .fill(null)
  //         .map((_, i) => (
